...
const parseLeadDescription = (description?: string | null) => {
  if (!description) {
    return {
      name: '',
      email: '',
  ...
  const getValue = (label: string) => {
    const match = description.match(new RegExp(`-\\s*${label}:\\s*(.*)`, 'i'));
    return match?.[1]?.trim() || '';
  };
...
const cleanDisplayValue = (value: string) => (value === '—' ? '' : value);

const normalizeLeadForView = (lead: Lead): Lead => {
  const display = getLeadDisplay(lead);

  return {
    ...lead,
    name: lead.name || cleanDisplayValue(display.name),
    email: lead.email || cleanDisplayValue(display.email),
    phone: lead.phone || cleanDisplayValue(display.phone),
    destination: lead.destination || cleanDisplayValue(display.destination),
    travel_dates: lead.travel_dates || display.travelDates || null,
    num_travelers: lead.num_travelers ?? display.travelers ?? null,
  };
};
...
const getLeadDisplay = (lead: Lead) => {
  ...
  const parsedTravelers = Number(parsed.guests);
  ...
  const destination =
    lead.destination ||
    parsed.tour ||
    (titleParts.length > 1 ? titleParts[0] : '') ||
    '—';
  ...
  return {
    ...
    travelDates: lead.travel_dates || parsed.date || '',
    travelers: lead.num_travelers ?? (Number.isFinite(parsedTravelers) ? parsedTravelers : null),
  };
};
...
export default function LeadsPage() {
  ...
  const leadsQuery: UseQueryResult<LeadsQueryResult, Error> = useQuery<LeadsQueryResult, Error>({
    queryKey: ['leads', user?.id, page, sourceFilter, statusFilter],
    queryFn: async (): Promise<LeadsQueryResult> => {
      const runQuery = async (options: {
        ...
        const buildQuery = () => {
          ...
          if (options.includeUserFilter && user?.id) {
            query = query.or(`user_id.eq.${user.id},user_id.is.null`);
          }

          if (options.includeSourceFilter && sourceFilter !== 'all') {
            query = query.eq('source', sourceFilter);
          }

          if (options.includeStatusFilter && statusFilter !== 'all') {
        ...
        const orderedResult = await buildQuery()
        ...
        return buildQuery().range(page * PAGE_SIZE, (page + 1) * PAGE_SIZE - 1);
      };

      let includeUserFilter = true;
      let includeSourceFilter = sourceFilter !== 'all';
      let includeStatusFilter = statusFilter !== 'all';
      ...
      for (let attempt = 0; attempt < 3; attempt += 1) {
        ...
        if (!result.error) {
          return {
            leads: (result.data as Lead[] | null) ?? [],
            count: result.count,
          };
        }
        ...
        if (includeUserFilter && errorMessage.includes('user_id')) {
          includeUserFilter = false;
          changed = true;
        }

        if (includeSourceFilter && errorMessage.includes('source')) {
          includeSourceFilter = false;
          changed = true;
        }

        if (includeStatusFilter && errorMessage.includes('status')) {
  ...
  const { data: leadsQueryData, isLoading, isError } = leadsQuery;

  const leads = leadsQueryData?.leads;
  const count = leadsQueryData?.count;
  const editingLeadDisplay = editingLead ? getLeadDisplay(editingLead) : null;
  ...
  const openEditSheet = (lead: Lead) => {
    setEditingLead(normalizeLeadForView(lead));
        ...
        <SheetContent className="sm:max-w-2xl flex flex-col h-full">
          <SheetHeader>
            <div className="flex justify-between items-center">
              <SheetTitle>{editingLeadDisplay?.name || editingLead?.title || 'Lead'}</SheetTitle>
            ...
            <SheetDescription>
              {editingLeadDisplay
                ? `Booked for: ${editingLeadDisplay.destination}. Contact: ${editingLeadDisplay.email} / ${editingLeadDisplay.phone}.`
            ...
            <div className="flex-1 overflow-y-auto">
              <TabsContent value="details" className="p-6">
                <div className="grid gap-4">
                  ...
                  <div className="grid grid-cols-4 items-center gap-4">
                    <Label htmlFor="destination-edit" className="text-right">Booked For</Label>
                  ...
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Select value={editingLead?.source ?? undefined} onValueChange={(value) => setEditingLead(prev => prev ? { ...prev, source: value } : null)}>
                      <SelectTrigger className="col-span-3"><SelectValue placeholder="Select a source" /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="Facebook">Facebook</SelectItem>
                        <SelectItem value="Instagram">Instagram</SelectItem>
                  ...
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Select value={editingLead?.status ?? undefined} onValueChange={(value) => setEditingLead(prev => prev ? { ...prev, status: value } : null)}>
                      <SelectTrigger className="col-span-3"><SelectValue /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="New">New</SelectItem>
                        <SelectItem value="Contacted">Contacted</SelectItem>
                  ...
                  {leads?.map((lead: Lead, index: number) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>{display.phone}</TableCell>
                        <TableCell>{display.destination}</TableCell>
